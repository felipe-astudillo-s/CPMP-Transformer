...
# --- [NUEVO] CLASE DE CODIFICACIÓN POSICIONAL ---
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=100):
        """
        Inyecta información sobre la posición relativa o absoluta de los contenedores.
        max_len: Altura máxima teórica que podría alcanzar una pila (100 es más que suficiente para CPMP).
        """
        super().__init__()
        ...
        # register_buffer asegura que 'pe' se mueva al dispositivo (CPU/GPU) correcto sin ser un parámetro entrenable
        self.register_buffer('pe', pe)

    def forward(self, x):
        # x shape: (batch_size * S_len, H_tensor, d_model)
        # Sumamos la codificación posicional correspondiente a la longitud actual (H_tensor)
...
class CPMPTransformer(Transformer):
    def __init__(self, H, C_dim, X_dim, d_model=64, nhead=8, num_layers=2, ff_dim_multiplier=4, dropout=0.1):
        ...
        # [NUEVO] Instanciamos el Positional Encoding
    ...
    def forward(self, S, X):
        ...
        # [NUEVO] Inyectamos la firma espacial ANTES de concatenar el token [CLS]
        # Así, el contenedor base es siempre la posición 0, el siguiente la 1, etc.
        ...
        stack_vertical_info = x[:, 0, :] # Extraemos solo el [CLS]
        ...
        # --- [MODIFICADO] LÓGICA DE TIGHT BOUNDING ---
        # Ahora asumimos que en tu nuevo rediseño de 'Stacks features' (vector X),
        # la característica que indica los "espacios libres" (free_spaces = H_real - size) 
        # está ubicada en el ÚLTIMO índice del vector X.
        # X shape: (batch_size, S_len, X_dim)
        
        free_spaces_idx = -1 # <--- Ajusta este índice según cómo ordenes tus variables en el generador de datos
        free_spaces = X[:, :, free_spaces_idx]
        
        # Un destino está lleno estrictamente si los espacios libres son 0 (o menos, por seguridad matemática)
