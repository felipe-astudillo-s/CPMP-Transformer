...
class DLTSLDSWDiscSolver(Solver):
    """
    DLTS-LDS Batched con discrepancia ponderada por log-probabilidad.

    Igual que DLTSLDSBatchedSolver pero cambia cómo se acumula la discrepancia:
    en lugar de sumar el rank (0, 1, 2, ...), suma la penalización logarítmica
    respecto al movimiento greedy:

        penalty = -log(prob / max_prob)

    Esto da penalización 0 al movimiento greedy, y penalización proporcional al
    gap de probabilidad para los demás. Dos desviaciones "casi-greedy" (prob≈max)
    acumulan menos discrepancia que una desviación fuerte (prob << max), por lo
    que el heap prioriza caminos más prometedores con mayor granularidad.

    max_disc sigue siendo el corte de discrepancia acumulada, pero ahora en
    escala logarítmica. max_disc=5 corresponde aproximadamente a una probabilidad
    acumulada de e^-5 ≈ 0.007 veces la del camino greedy.
    ...
    p : float        MP-Constant branch pruning [0, 1]
    k : int          Cada cuántos niveles recalcular lower bound
    d : float        Factor de escala del bounding model
    z : int          Profundidad hasta la cual la discrepancia no se acumula
    time_limit : float
    max_disc : float | None   Penalización log acumulada máxima (None = sin límite)
    batch_size : int          Máximo de layouts por forward pass
    """
    ...
    def __init__(self, branching_model, bounding_model=None, bounding_adapter=None,
        ...
        super().__init__("DLTSLDSWDiscSolver")
    ...
    def solve_from_layout(self, layout, H, max_steps):
        ...
        Q = []
        heapq.heappush(Q, (0.0, counter, root.steps, root, float('-inf')))
        ...
        while Q and (time.monotonic() - start_time) < self.time_limit:
            ...
            # Con floats usamos tolerancia para agrupar nodos del mismo nivel
            level_nodes = []
            while Q and Q[0][0] == current_disc:
                disc, _, depth, state, hlb = heapq.heappop(Q)
                level_nodes.append((depth, state, hlb))

            # Pre-filtrar: nodos terminados y poda por depth/hlb
            to_process = []
            ...
            for chunk_start in range(0, len(to_process), self.batch_size):
                ...
                # --- 4. Generar hijos con discrepancia ponderada por log-prob ---
                for (depth, state, hlb), ranked in zip(chunk, ranked_list):
                    if not ranked:
                        continue

                    max_prob = ranked[0][1]
                    threshold = self._mp_constant(max_prob, self.p)

                    for (src, dst), prob in ranked:
                        if prob < threshold:
                            break
                        ...
                        # Penalización logarítmica: 0 para greedy, mayor para peores movimientos
                        if depth >= self.z:
                            penalty = -math.log(prob / max_prob + 1e-12)
                        else:
                            penalty = 0.0
                        child_disc = current_disc + penalty
    ...
    # ------------------------------------------------------------------
    # Inferencia batch (idéntica a DLTSLDSBatchedSolver)
    # ------------------------------------------------------------------
    ...
    def _get_probabilities_batch(self, layouts, H):
        ...
        with torch.no_grad():
            logits = self.branching_model(*batched)
    ...
    def _get_bounding_estimate_batch(self, layouts, H):
        ...
        with torch.no_grad():
            log_costs = self.bounding_model(*batched)
            costs = torch.exp(log_costs).view(-1)
